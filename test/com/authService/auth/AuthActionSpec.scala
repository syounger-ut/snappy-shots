...
class AuthActionSpec extends AsyncUnitSpec {
  ...
  def prepareResponse(headers: Headers = Headers()): Future[Result] = {
    ...
    val authAction = new AuthAction(controllerComponents.parsers.default, mockAuthService)(controllerComponents.executionContext)
  ...
  describe("when an auth token is provided") {
    ...
    describe("when the token is valid") {
      it("should return ok status") {
        ...
        val subject = prepareResponse(Headers("Authorization" -> s"Bearer ${mockJwtToken}"))
